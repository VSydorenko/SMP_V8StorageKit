...
function Get-PendingVersions {
    ...
    # Порожній звіт сховища (жодної версії) сам собою не підтверджує, що версію
    # $LastSynced колись справді було залито — це так само розбіжність, як і випадок
    # вище, лише з іншого боку: там git випереджає сховище з непорожнім звітом, тут
    # звіт узагалі нічого не каже про версію, яку storage.json уже вважає залитою.
    # $LastSynced -eq 0 — легітимний стан щойно підключеного продукту (сховище ще
    # порожнє, і "Нових версій немає" — правильна відповідь), тому виняток лише для
    # $LastSynced -gt 0.
    if ($AllVersions.Count -eq 0 -and $LastSynced -gt 0) {
        throw "Звіт сховища порожній (жодної версії), а storage.json стверджує, що вже залито " +
              "версію $LastSynced. Порожній звіт не підтверджує це — сховище могло стати " +
              "недоступним чи звіт пошкодженим. Синхронізацію зупинено, розберіться з " +
              "розбіжністю вручну."
    }
