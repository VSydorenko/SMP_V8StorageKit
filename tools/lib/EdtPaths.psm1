...
function Convert-KitFormTail {
    ...
    if ($Rest.Count -eq 1 -and $Rest[0] -like '*.dcssca') {
        # Знахідка пілота на SMP_SimplyCatalog (2026-09-10), доведена З ОБОХ СТОРІН — тим самим
        # стандартом доказу, що й таблиця макетів:
        #   EDT-сторона: 674 файли `.dcssca` по восьми деревах парку, усі на рівні форми
        #     (`Forms/<Ф>/<Ім'я>.dcssca`), а не під `Attributes/`;
        #   Designer-сторона: у живих деревах SMP_BankExchange і SMP_SimplyConnect окремих файлів
        #     `.dcssca` НУЛЬ, натомість 11 `Ext/Form.xml` містять елемент `<ConditionalAppearance`.
        # Отже це той самий клас, що й `Attributes/…`: умовне оформлення форми вбудовується в
        # Ext/Form.xml, окремого Designer-шляху для нього не існує. Тобто Unmapped ("знаємо, що
        # не потрібне"), а не Unresolved ("не знаємо, що це").
        #
        # Чому це не косметика: доти воно падало в Unresolved, який НІКОЛИ не видаляється, — і
        # тека джерела після перейменування не порожніла. У парку це 674 осиротілі EDT-файли,
        # що лишились би в репозиторіях назавжди (на пілоті — рівно один, бо розширення мале).
        #
        # Розширено НЕ ширше за доказ: сусідній `.oform` (19 файлів у парку) обох сторін не має
        # і свідомо лишається Unresolved.
        return New-KitEdtUnmapped -Reason "умовне оформлення форми ('$($Rest -join '/')') вбудовується у Ext/Form.xml форми — окремого Designer-файлу немає (доведено обома сторонами, див. коментар)."
    }
...
function Get-KitEdtRenamePlan {
    ...
    # M-7 (фінальне рев'ю блоку): обхід дерева написано ТУТ ЗАНОВО, а не через Get-KitRelativeFiles
    # (TreeCompare.psm1), і це свідома відмова, а не недогляд — CLAUDE.md окремо забороняє другу
    # реалізацію наявної перевірки, тож вона мусить бути підписана. Причина: Get-KitRelativeFiles
    # фільтрує $script:PlatformJunk і не має -Force, тобто МОВЧКИ викидала б файли з обліку — рівно
    # те, від чого стоїть інваріант цієї функції («перелік Moves/Unmapped/Unresolved — повний опис
    # дерева, жоден файл не зникає без показу людині»). Відфільтрований там файл не потрапив би ні
    # в Unmapped, ні в Unresolved, а лишився б на диску після перейменування без жодного сліду.
    #
    ...
    #
    # Чесно про межу (знахідка G, рев'ю раунду 4): за ПОТОЧНИМ потоком це ТАВТОЛОГІЯ — цикл вище
    # кладе кожен обійдений файл рівно в один список, тож спрацювати інваріант може лише після
    # майбутньої зміни коду між обходом і чотирма списками. Він тут саме як запобіжник на таку
    # зміну, а не як діюча перевірка нинішнього потоку. Тест, що це доводить (мок групування, який
    # губить один кандидат), — EdtPaths.Tests.ps1, "знахідка G".
