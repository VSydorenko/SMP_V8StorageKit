...
if (-not $Command -or $available -notcontains $Command) {
    ...
    # хвилясте підкреслення) суперечить охайному [-]-виводу самого check. Код 2 тут —
    # диспетчерська відмова ще до виклику команди. Контракт кодів виходу команд (план
    # наступного блоку, не вигадувати заново): 0 — виконано; 1 — зупинка (throw) або
    # check із помилками; 2 — sync: дзеркало оновлено, злиття не виконано; 3 — verify:
    # є що робити. Тобто 2 і 3 — коди команд, які ВІДПРАЦЮВАЛИ, а не «не запустились
    # узагалі»: код виходу сам не розрізняє «диспетчер відмовив» від «команда завершилась
    # частковим успіхом» — розрізняти треба з тексту виводу.
...
try {
    ...
    # зібрані check-ом знахідки. Код 2 тут — та сама диспетчерська відмова, що й вище:
    # команда впала до того, як повернула структурований результат. Контракт кодів
    # виходу команд — у коментарі вище; той самий код 2 повертає й сама sync за штатного
    # часткового успіху (ExitCode самої команди, нижче) — код їх не розрізняє.
