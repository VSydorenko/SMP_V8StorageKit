...
function Invoke-KitRenameEdt {
    ...
    .DESCRIPTION
        ...
        Обсяг коміта перейменування — рівно два піддерева (`git commit --only -- <джерело>
        [<ціль>]`, знахідка I-5 фінального рев'ю) ПЛЮС усе, що застейджить у тимчасовий індекс
        `pre-commit`-хук самого споживача: при частковому коміті git виконує хук із
        GIT_INDEX_FILE = тимчасовий індекс, тож доданого там `--only` не відсіює. Обійти можна
        лише `--no-verify`, тобто відключенням чужих хуків — kit цього не робить свідомо.
        У нашій конфігурації вікно закрите НЕ командою, а налаштуванням: `install-hooks` ставить
        `core.hooksPath .githooks`, чим вимикає власний `.git/hooks/pre-commit` споживача, а
        роздаваний kit `templates/githooks/pre-commit` нічого не стейджить — лише читає
        `symbolic-ref HEAD` і блокує коміти в `storage/*`, на `main` виходячи нулем.
    ...
    # ПЕРЕД друком Unmapped, і оператор, що робить прев'ю на дереві з колізіями, не бачив ЖОДНОГО
    ...
    try {
        ...
        try {
            ...
            # Свідома плата за `--only`, і вона ширша, ніж «незастейджена правка людини»
            # (уточнено пере-рев'ю фінальної хвилі — попередня редакція цього коментаря це
            # занижувала): для НАЗВАНИХ шляхів git бере вміст РОБОЧОГО ДЕРЕВА й перехешовує його
            # через фільтри цільового шляху. У парку індекс і диск не збігаються НА ЖОДНОМУ
            # текстовому файлі — gitsync лишив на диску CRLF при LF в індексі (`git ls-files --eol`
            # на SMB_ukr_vendor: `i/lf w/crlf`). Отже блоби коміту перейменування — НОВІ об'єкти,
            # відмінні від gitsync-історії кінцями рядків, на кожному файлі кожного репозиторію.
            #
            # Виміряно на моделі, відтвореній із живого парку: виявлення перейменування падає
            # R100 -> R098, але `git log --follow` дає ті самі 4 коміти, а `git blame -w` через
            # межу — ті самі 1995 із 2000 рядків. На дрібних файлах (1..100 рядків) — R095.
            # Тобто центральна властивість блоку (безперервність --follow і blame -w) НЕ слабшає;
            # різниця «лише кінці рядків» не опускається нижче порога 50 % на жодному розмірі.
            #
            # ВАЖЛИВО, і це ніде більше не записано: чистоту цього коміта тепер тримає запобіжник
            # Test-GitTextPolicy на -TargetRoot. Під `text=auto` замість `-text` та сама форма дає
            # R066 і блоб, нормалізований у LF, — тобто послаблення того запобіжника тихо вбиває
            # `--follow`, а не лише побайтову рівність кроку 5, як вважалося досі.
            #
            # Розширення обсягу — лише на ці два шляхи; будь-що поза ними (застейджене чи ні)
            # коміт більше не бачить, і саме це й було метою I-5.
