...
> **Скілів із колонки «Скіл» ще немає — вони з'являться в наступному блоці розробки kit.**
> Голе звернення до них дасть `Unknown skill`, тож доти працюйте **командами з правої
> колонки**. Команди `sync`, `dump`, `verify`, `provision`, `canon`, `build`, `check` і
> `session-check` існують і роблять саме те, що описано; рядки `reconcile` і `finish` — це
> послідовності з тих самих команд, які поки що виконуються вручну, а `migrate` команди ще не
> має взагалі. Колонка «Скіл» — карта того, куди ці наміри переїдуть, а не список доступного
> зараз. Окремо: `v8storagekit:storage-pipeline`
> (він є, але описує канон **до** 1.0) і `v8storagekit:product-onboarding` /
> `v8storagekit:repo-migration`, які теж чинні лише для старої конвенції.

| Намір | Скіл (з'явиться пізніше) | Команда kit — чинна вже зараз |
|---|---|---|
