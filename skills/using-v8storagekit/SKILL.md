...
Окремої команди чи скіла для переходу зі старої схеми немає навмисно: усі вхідні форми —
порожній репозиторій, gitsync-вивантаження сховища, напівмігрований 0.6.0 — веде `onboarding`.

Дев'ять команд kit: `check`, `session-check`, `sync`, `dump`, `verify`, `canon`, `provision`,
`build`, `install-hooks`. Запускаються з кореня репозиторію:
...
він лише друкує базу, користувача й ціль (`dump.psm1`, гілка без `-Apply`, `continue` до
запуску Конфігуратора). `-Apply` — лише на явне прохання людини.
