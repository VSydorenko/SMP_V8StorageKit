...
- **Скіл нічого не запускає без дозволу** користувача чи harness-запиту: `-Apply` лише на явне
  прохання (дієслово-наказ або підтвердження після прев'ю). Прев'ю **`sync` і `verify`** піднімає
  платформу (хвилина-дві, ліцензія) — скіл каже це людині перед запуском. **`dump` — ні:**
  `dump.psm1` у гілці `-not $Apply` друкує базу, користувача й ціль і робить `continue`, а
  `Invoke-V8Designer` стоїть нижче (перевірено в коді, знахідка рев'ю B5). Ця константа була
  неправильна й устигла розійтись у три місця — стверджувати «піднімає платформу» про команду, не
  подивившись, де саме стоїть виклик платформи, тут більше не можна.
...
**Ризик:** `спільний код` — додає команду в диспетчер і змінює `core.hooksPath` у репозиторії споживача — від неї залежать
усі хук-перевірки B1–B2.
...
**Ризик:** `механічна` — текст скіла, не код. Межі («лише на явне прохання», «сховище тільки читання») тримає
**guard-тест**, а не уважність рев'юера — рев'ю перевіряє, що guard-тест на ці рядки є
й падає без них, а не вичитує формулювання. Guard-тести з цієї задачі — інструмент для
усіх наступних; їх рев'ю варте уваги, тексти скілів — ні.

**Files:**
- Create: `skills/onboarding/SKILL.md`
- Create: `tools/tests/Skills.Tests.ps1`

**Чистий репозиторій — головний випадок цього скіла, а не крайній** (знахідка живого прогону B4).
Префлайт на будь-якому непідключеному репозиторії каже: «скопіюйте `templates/v8storagekit.yaml.example`,
**заповніть** і запустіть `kit check` ще раз». На напівмігрованому це змістовна порада — значення
беруться зі `storage.json` і `v8project.yaml`. На **чистому** брати їх нізвідки: ні імен source-set-ів,
ні шляхів сховищ, ні імен розширень. Тобто онбординг чистого репозиторію **важчий** за онбординг
напівмігрованого, а повідомлення в обох випадках однакове.

Скіл мусить відповідати саме на цей випадок: звідки взяти ключі джерел і шляхи сховищ, коли в
репозиторії немає жодного файла kit — питати людину поіменно, а не пропонувати «заповнити». Порада
префлайту при цьому лишається як є: `Preflight.psm1` — код B1, і розводити там дві поради за
формою репозиторію дорожче, ніж один раз добре описати шлях у скілі, куди префлайт і відсилає.
...
3. Стан: `pwsh -NoProfile -File "${CLAUDE_PLUGIN_ROOT}/tools/kit.ps1" check -RepoRoot .`
   ...
   - жодного маркера — **порожній репозиторій**: новий проєкт або клієнтська база (розділ 5);
   - **`DT-INF/` поруч із текою вихідників і без `storage.json`** — надійна ознака (спека §9.1);
     `AUTHORS`, `VERSION`, `ConfigDumpInfo.xml` уточнюють, але можуть бути відсутні або лежати
     всередині `src/` (`SMB_ukr_vendor` — перевірено на живому дереві). Найімовірніше це
     **gitsync-вивантаження сховища** (розділ 5.1), і для нашого парку це так — але
     **`DT-INF/` доводить лише формат дерева, не його походження**. Тому скіл **показує, що бачить,
     і питає, яке сховище за цим стоїть**; підтверджує це проба доступу до сховища та його звіт, а
     не сам вигляд дерева. Стверджувати походження з формату — та сама помилка, від якої пішли,
     лише з іншим знаком.
     **Шукати всі такі теки, не першу**: у парку є репозиторій із двома
     (`SMP_SMB_rus_DEV` — конфігурація плюс розширення), і це два джерела з різними `truth` і
     різною глибиною, а не одне;
   - `storage.json` поруч із `v8project.yaml` — **напівмігрований 0.6.0** (розділ 5.2).
...
3. **Коміт перейменування:** кожен файл EDT-дерева → його точний Designer-шлях за таблицею
   відповідності видів метаданих; вміст поки EDT-ний.
   **Сказати людині час ДО запуску, а не на двадцятій хвилині:** повний прохід — **20–40 хвилин на
   дерево** (виміряно: 22 хв 38 с на 18,7 тис. файлів), бо `git mv` викликається на кожен файл
   окремо — пакетного еквівалента для довільних пар «шлях → шлях» git не має. `SMP_SMB_rus_DEV`
   більший і має **два** дерева. Збій безпечний (відкат `reset --hard`), але не дешевий: після
   нього прохід починається з нуля. Без цього попередження людина вирішить, що команда зависла, і
   вб'є її посеред проходу.
   **Після зірваного проходу й відкоту дерево джерела git-чисте, але побайтово інше — це норма, не
   поломка.** `git checkout <SHA> -- src` матеріалізує файли **через фільтри**, а gitsync лишив на
   диску CRLF при LF в індексі (`core.autocrlf=input`, `text: auto`): виміряно 188 із 195 файлів
   змінили байти, при цьому `git status` порожній. На маршрут це не впливає, і механізм перевірено,
   а не припущено: `git mv` переносить запис **індексу**, а індекс уже тримав LF, тож блоби коміту
   перейменування ті самі незалежно від відкоту; коміт конверсії (крок 5) усе одно заміщає вміст
   дампом зі сховища. Вимога `-text` зі спайку стосується **цілі** й коміту конверсії, не джерела.
   Сказати це людині заздалегідь: інакше вона відкриє джерело в редакторі, побачить інші кінці
   рядків і почне «лагодити» те, що працює. Файли без відповідника (`DT-INF/`,
...
5. **Коміт конверсії на `main` — плоский, не злиття:** тека джерела стає побайтово рівною вершині
   `storage/<ключ>`, включно з видаленням об'єктів, яких у сховищі вже немає. **Перевірити це, а не
   припустити:** `git rev-parse main:<шлях>` має дорівнювати `git rev-parse storage/<ключ>:<шлях>` —
   рівність хешів дерев доводить побайтову ідентичність усього піддерева. Не збіглося — зупинка:
   найімовірніше git сконвертував кінці рядків (політики з кроку 1), і далі `verify` покаже
   розбіжність на кожному файлі, яку легко списати на сховище.
...
Ціна неправильного порядку виміряно спайком (Task 1, протокол `docs/migration/2026-09-09-history-spike.md`, коміт `dee3f53`):
...
`blame` і плоский `log`, і саме вони — причина порядку. Результат правильного порядку:
...
| `<ws>/v8project.local.yaml` | `devInfobase:` | `infobases.<ключ>` накладки kit |
...
**Ризик:** `механічна` — текст скіла, не код. Межі («лише на явне прохання», «сховище тільки читання») тримає
**guard-тест**, а не уважність рев'юера — рев'ю перевіряє, що guard-тест на ці рядки є
й падає без них, а не вичитує формулювання.
...
1. **Нові версії зі сховища → дзеркало** (прев'ю, потім `-Apply` **на явне прохання**). Те, що
   людина щойно комітила у сховище, — це **контекст, а не дозвіл**: вона сказала, що зробила, а не
   що робити далі. Виводити дозвіл із обставин заборонено глобальним правилом, і скіл цю межу
   переказує, а не перевизначає (знахідка рев'ю B5):
   ```
...
**Частина А: скіли, довідка, шаблони**
...
```bash
git add -A -- skills templates docs/migration tools/tests/Templates.Tests.ps1
git commit --only -- skills templates docs/migration tools/tests/Templates.Tests.ps1 -m "B5: вилучено старі скіли; довідка про legacy-міграцію; templates/CLAUDE.md і README під модель 1.0"
```

---

**Частина Б: `docs/unica-contract.md` A8–A11; `CLAUDE.md` kit — структура**

- [ ] **Step 6:** У таблицю розділу A `docs/unica-contract.md` після A7 додати рядки (дослівно зі спеки §11):
...
- [ ] **Step 6в: порада префлайту відсилає в неіснуючий `migrate`** (знахідка живого прогону B5)

`Preflight.psm1:79` на репозиторії 0.6.0 друкує «Правильний шлях — скіл `v8storagekit:migrate`
(команда `kit migrate`)… з'явиться в наступному блоці розробки kit», `check.psm1:358` — «(перекладе
`kit migrate`)», а `Preflight.Tests.ps1:76` **пінить саме цю пораду** (`Should -Throw
'*0.6.0*kit migrate*'`). За чинною §9 `migrate` не буде **ніколи**: усі вхідні форми веде
`onboarding`.

**Це правиться тут, а не чекає B8** — і теза Task 2 «пораду префлайту не чіпаємо» цьому не
суперечить: вона про **дві поради за формою репозиторію** (загальна замість точної), а тут порада
називає скіл і команду, яких не існуватиме. Наслідок не теоретичний: на єдиному репозиторії цієї
форми `check` відсилає в нікуди й **суперечить скілу, який агент щойно відкрив** — §5.2 `onboarding`
каже «це моя територія», `check` каже «іди в `migrate`». Три блоки такого розходження коштують
дорожче за три рядки.

- `Preflight.psm1` — порада на `v8storagekit:onboarding`, без обіцянки «з'явиться в наступному
  блоці»: скіл уже є. Згадка `storage.json` лишається — вона правильна й корисна.
- `check.psm1:358` — «перекладе скіл `v8storagekit:onboarding`».
- `Preflight.Tests.ps1:76` — перецілити асерт тим самим рухом. **Не видаляти**: він тримає
  властивість «на репозиторії 0.6.0 порада називає шлях», а не текст `migrate`.
- **Перевірка ширша за guard скілів:** `grep -rn 'kit migrate\|v8storagekit:migrate' tools/ templates/ skills/`
  — порожньо, крім навмисних пояснень «`migrate` немає, ось чому». Guard із Task 2 дивиться лише в
  `skills/`, тож `tools/` він не покрив би — саме тому дефект і дожив до живого прогону.
...
- [ ] **Step 8: Живий прогін блоку** — єдина точка B5, де відкривається сесія
  (локальний маркетплейс, за підтвердженням користувача): у синтетичному
