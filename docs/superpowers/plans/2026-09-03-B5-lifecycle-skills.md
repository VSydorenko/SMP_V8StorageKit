...
3. Стан: `pwsh -NoProfile -File "${CLAUDE_PLUGIN_ROOT}/tools/kit.ps1" check -RepoRoot .`
   ...
   - `*.mdo` у деревах джерел — **правило переписується** (§9, 2026-09-09): це не «поза контуром»,
     а gitsync-вивантаження сховища, тобто головний вхід міграції. Текст цього пункту чекає нової
     редакції §9; до неї — зупинитись і показати знайдене людині.
...
> **⚠ §9 ПЕРЕПИСУЄТЬСЯ — текст нижче не виконувати до нової редакції** (2026-09-09).
>
> Рішення `2f2da62` про EDT виявилось помилковим, користувач поправив архітектора напряму:
> **вісім «EDT-репозиторіїв» у `R:\github` — це gitsync-вивантаження СХОВИЩ** (квартет
> `AUTHORS`+`VERSION`+`DT-INF/`+`ConfigDumpInfo.xml`, зроблені колись «для історії»), а не
> EDT-розробка. Істина для них — сховище, і саме вони **головний вхід міграції**, а не «поза
> контуром». Конверсія EDT → Designer не потрібна: старий скіл `repo-migration` робив саме це
> (orphan-гілка, реплей зі сховища, старе дерево не чіпається) і пройшов на `BankExchange` та
> `SimplyConnect`.
>
> З `8756de0` чинним лишається тільки **«не окрема команда»**. Три вхідні форми в новій редакції:
> порожній репозиторій зі сховищами (новий проєкт), gitsync/EDT-репозиторій (вісім), напівмігрований
> 0.6.0 (два, тимчасово) — усі в діалозі з людиною. Правило «`*.mdo` → зупинитись» **знімається**.
>
> Архітектор брейнштормить редакцію з користувачем. Довідку зі старого `repo-migration` **не
> видаляти** — вона стане основою.
...
Реплей наново тут не жертва, а виграш (спека `818106e`): він дає **повну** історію в
`storage/<ім'я>`, якої частковий реплей від `N+1` не дав би, коштує хвилини для сховищ розширень і
виконується двічі за весь перехід. Якщо реплей здається довгим — це не привід переносити лічильник.
