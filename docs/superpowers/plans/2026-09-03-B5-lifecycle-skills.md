...
> **Загальні правила розробки kit переїхали у проєктний скіл `kit-dev`**
> (`.claude/skills/kit-dev/` — `SKILL.md` за моментом застосування, `references/cases.md` із
> випадками, датами й ціною). Причина форми: це **процедури з моментом дії** («перед новою
> перевіркою — `grep` за наявною», «мутуй лише на копії», «ділиш файл — відповідаєш за посилання
> **в** нього»), а процедуру читають тоді, коли її активують, — не тоді, коли про неї згадали.
> Скіл лежить у `.claude/skills/`, не в `skills/`: усе в `skills/` кореня плагін роздає
> споживачам, а це знання для того, хто розробляє сам kit. Видима межа — префікс: `v8storagekit:`
> роздається, голе ім'я `kit-dev` живе тільки тут.
>
> **Нижче лишились правила про виконання самих планів** — вони в скіл не пішли свідомо, бо
> адресовані авторові й контролеру блоку, а не тому, хто пише код kit. Вирізано звідси лише те,
> чий зміст знайдено в скілі поіменно; сумнівне лишено на місці. Ціна помилки тут асиметрична:
> зайвий абзац у закритому плані нікому не заважає, а вирізаний і ніде не збережений зникає
> назавжди.
...
Ціна неправильного порядку виміряно спайком (Task 1; метод і числа — `docs/migration/legacy-gitsync-repo.md`, розділ 12; повний журнал прогону — коміт `dee3f53`):
