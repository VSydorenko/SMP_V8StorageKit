...
**Це давній пробіл самої функції, а не наслідок нового `git reset`** (перевірено живим git, і
формулювання тут важливе для того, хто читатиме історію): безумовний `git add -A -- <шлях>`, який
стоїть у `adopt` після заміни, перезаписує запис індексу так само безповоротно — втрата
відбувалася б і без `reset`, лише пізніше. Тож шукати дефект треба в `Backup-KitDirtyFiles`, не
в механізмі ancestry.

Пробіл ширший за `adopt`: він є і в `canon`, але вікно втрати там вужче — `canon` **сам не
комітить**, тож застейджена правка доживає в індексі до коміту людини й гине лише якщо та зробить
`git add -A`. В `adopt` втрата детермінована, бо команда комітить сама. Тому функція додатково
виконує:
...
| `skills/using-v8storagekit/SKILL.md:27, 37` | таблиця маршрутизації (`reconcile` = `sync` → `canon` → `merge storage/*`) і рядок «Десять команд kit» без `adopt` | переписати під заміну; команд тепер одинадцять |

**Восьме місце — вступний скіл, і воно найважливіше з усіх.** `skills/using-v8storagekit/SKILL.md`
вантажить хук старту сесії в **кожній** сесії репозиторію-споживача: це перше, що читає агент.
Скіл, який описує знятий контур, навчає хибного маршруту раніше, ніж агент дійде до `reconcile`.
Крім того, влучання саме в цей файл дає **власна приймальна перевірка цієї задачі** (Step 4а) —
лишити її червоною не можна: наступний виконавець прочитає червону перевірку як «вона й так
червона, норм».
...
**І там, і там дописати, чому (а) і (б) не зводяться до одного** (вимога архітектора,
2026-09-17). Після заміни вони починають частково перетинатись, і перше ж рев'ю спокусливо
запропонує їх злити — тоді гейт мовчки втратить половину покриття. Формулювання для обох місць:

> Критерії (а) і (б) доповнюють один одного і навмисно не зводяться до одного: (а) відповідає на
> питання «чи хтось працював у тому самому місці», (б) — «чи частина роботи агента не ввійшла у
> сховище». Жоден не покриває іншого. Агент **додав** файл, якого у сховищі немає: при `adopt`
> він зникне, хоча нові версії його не торкались — ловить лише (б). Агент змінив файл, і людина
> цю зміну у своїй версії **врахувала**: з гілки нічого не зникає, хоча чужа робота лягла рівно
> туди, де агент працював — ловить лише (а).
...
```bash
grep -rn 'merge storage/\|merge --no-ff storage/' docs/ skills/ templates/ --exclude-dir=superpowers
```

`--exclude-dir=superpowers` обов'язковий: плани `B2`/`B4`/`B5` і спеки описують стан на момент
свого написання й **навмисно** лишаються з історичними формулюваннями — правити їх означало б
переписувати історію рішень. Без виключення перевірка дає постійний шум і перестає бути
перевіркою.

Expected: рівно одне влучання — `skills/verify/SKILL.md:58`
(`Злиття … не вдалося (конфлікт …) … git merge storage/…`). Воно про звірочний коміт у **головну**
гілку й лишається як є: це та сама межа, що в абзаці «Межа, яку цей план НЕ переходить». Жодного
влучання про гілку задачі.
...
`<воркспейс>/build/artifacts` у кореневу теку. Правиться лише текст — у **чотирьох** місцях, і
`build.psm1` серед них тримає абсолютний шлях **тричі**: підказка (рядок 108), повідомлення «у
теці порожньо» (рядок 152) і `.DESCRIPTION` (рядки 79–80). Повний перелік:

| файл | що правимо |
|---|---|
| `tools/commands/build.psm1` | три місця: підказка, «порожньо», `.DESCRIPTION` |
| `templates/CLAUDE.md` | приклад для споживача |
| `skills/finish/SKILL.md` | крок «Артефакти» (після перенумерації C1 — крок 7) |
| `skills/using-v8storagekit/SKILL.md:49` | вступний скіл — його читає кожна сесія споживача |

Приймальна перевірка задачі:

```bash
grep -rn 'output=<корінь\|output=.*build/artifacts' tools/ skills/ templates/ | grep -v 'відносний до воркспейсу'
```

Expected: порожньо — жодного абсолютного `output=`.
...
- [ ] **Step 4а: Полагодити тести маніфесту — і перевірити, що вони падають з ПОТРІБНОЇ причини**

Маніфест будують 31 місце в 10 файлах тестів. Більшість полагодить Step 4 (фікстура), решта —
дописуванням `kitVersion: 1.0.1` у власний `ManifestText`. Але «дописувати, поки не позеленіє» —
**небезпечна вказівка**: після `Required @('version','kitVersion','workspaces')` маніфест без
поля кидає `бракує обов'язкового ключа 'kitVersion'` **раніше** за будь-яку іншу перевірку, і
асерція, чий патерн випадково збігається з цим текстом, лишиться зеленою, перевіряючи вже не те.

Два місця в `tools/tests/Manifest.Tests.ps1` саме такі:

**(A) рядок 89 — «невідомий кореневий ключ»** асертить перелік дозволених ключів дослівно:

```powershell
{ Read-KitManifest -Path $p } | Should -Throw "*'products'*version, product, client, mainBranch, workspaces*"
```

Після вставки `kitVersion` у `-Allowed` перелік у повідомленні стає
`version, kitVersion, product, client, mainBranch, workspaces` — патерн не збігається, тест
падає. Оновити патерн під новий перелік. Це не `ManifestText`-проблема, тож «дописати
`kitVersion`» його не лікує.

**(B) рядок 105 — «version: 2 — зупинка»** будує маніфест без `kitVersion` і чекає `'*version*'`.
Після Task 8 зупинка станеться на `kitVersion`, до перевірки `version: 2`, — а патерн
`'*version*'` збігається і зі словом `kitVersion`. **Тест лишиться зеленим і перестане перевіряти
`version: 2` взагалі.** Лікування подвійне: дописати `kitVersion: 1.0.1` (щоб `version: 2` став
єдиним дефектом) **і** звузити асерцію до `'*version: 2*'`, щоб вона не могла збігтися з
повідомленням про `kitVersion`.

**Правило замість «дописувати, поки не позеленіє»:** маніфест у тесті отримує `kitVersion`; а
якщо тест навмисно перевіряє **відмову схеми**, перечитати його асерцію й переконатись, що вона
й далі падає з потрібної причини, а не з нової.

**Покриття перевірки (kit-dev, case 12б).** Небезпечних місць — рівно ці два. Перевірено:
всі 31 побудову маніфесту в 10 файлах; усі 26 асерцій `Should -Throw` у восьми файлах, що
будують власний маніфест (жодна інша не містить підрядка, здатного збігтися з
`бракує обов'язкового ключа 'kitVersion'` — найближчий, `'*Storage-Version*5*'` у
`Sync.Tests.ps1:503,510`, вимагає `Storage-Version` цілком); обидва тести, що перевіряють лише
код виходу без тексту (`Provision.Tests.ps1:390`, `V8.Tests.ps1:109`) — обидва йдуть через
`New-KitFakeRepo`, тож Step 4 їх покриває.

- [ ] **Step 5: Прогнати**

Expected: PASS. Кожне падіння читати як питання «з потрібної причини?», а не як «чого ще
бракує» — див. Step 4а.
