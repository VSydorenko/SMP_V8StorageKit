...
- Версію плагіна цей план **не піднімає** — бамп і `kitVersion` належать блоку C5.
...
- не додає `kit adopt` (блок C2, спека §5);
- не чіпає `origin` (блок C3, спека §6);
- не править текстів про артефакти (блок C4, спека §7);
- не вводить `kitVersion` і не піднімає версію плагіна (блок C5, спека §8);
