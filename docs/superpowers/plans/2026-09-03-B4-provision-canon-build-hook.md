...
- **Тести без платформи:** `pwsh -NoProfile -File tools/tests/Run-Tests.ps1 -ExcludeTag Integration`.
  `Integration` (`provision` порожня і з `.dt`, `canon` round-trip) — лише з підтвердженням.
...
### Task 5: вилучення `load-ext.ps1`, `build.ps1`, `Read-V8LocalConnection`; перехідні позначки
...
git commit -m "B4: вилучено load-ext.ps1, build.ps1 і Read-V8LocalConnection; перехідні позначки під kit provision/canon/build"
```

---

### Task 6: жива перевірка хука в сесії (за підтвердженням користувача)

Маркетплейс зареєстровано локально (B1 Task 10), тож плагін у реєстрі вказує на робочу копію.

- [ ] **Step 1:** У синтетичному репозиторії з `Sync.Tests` (Integration) або на копії
...
- [ ] **Step 2:** Там же `kit check` — знахідок `hook-shim` немає.
- [ ] **Step 3:** У репозиторії **без** плагіна в реєстрі (тимчасово перейменувати ключ у копії
...
| Integration: `provision` порожня і з `.dt`; `canon` round-trip; `build .epf` | 1–3 |
