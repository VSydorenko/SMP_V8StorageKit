...
`verify` без `-Apply` усе одно піднімають платформу. `dump` — **ні**: без `-Apply` він лише друкує
базу, користувача й ціль (перевірено в `dump.psm1`, знахідка рев'ю B5). `-Apply` — лише на явне
прохання людини.
...
`type: EXTERNAL_DATA_PROCESSORS` у `v8project.yaml`, гілка `.cfe` вилучена (`operation=make`); живий
прогін на `SMP_BankExchange` знайшов усі 12 реальних дескрипторів після перейменування теки
`epf` → `processors`, тобто дискримінатор за типом source-set працює не лише у фікстурі».

`docs/follow-ups.md`, **новий пункт** (знахідка живого прогону B4, свідомо відкладена): подача
відмови префлайту різна в різних команд — `check` дає заголовок, `[-]`-рядок і підсумок;
`session-check` те саме; `provision` і `build` — **голий рядок** із `catch` диспетчера
(`Write-Host $_.Exception.Message`, B1), без заголовка, маркера й підсумку. Розходження системне й
успадковане: заголовок і `[-]` дає `-Lenient`-режим `check`, голий рядок — диспетчер. Нові команди
його лише зробили помітним, бо їх стало вісім. **Чому відкладено:** уніфікація подачі зачіпає всі
вісім команд і всі їхні тести на текст виводу, а виграш — охайність: команди з голим рядком
(`provision`, `build`) хук агентові не віддає, він кличе лише `session-check`, у якого маркери є.
Брати в роботу — рішення користувача.
...
- [ ] **Step 2б: `devInfobase:` не має мовчати** (знахідка живого прогону B4)

Task 5А вилучає `Read-V8LocalConnection` разом із конвенцією `devInfobase:` у `v8project.local.yaml`.
Але в **живих** репозиторіях цей ключ є: прогін на `SMP_BankExchange` показав три воркспейси з
реальними підключеннями до баз людини. Після вилучення його не читає ніхто, і `local-audit` у
`check` цілиться в `infobase:` — отже `Read-V8ProjectLocalInfobase` поверне `$null`, гілка аудиту
пропуститься **без жодного рядка**, і на таких репозиторіях перевірка §2.5 не спрацює ніколи й мовчки.

Форма дефекту та сама, яку блок закривав уже двічі: **«нічого не сказано» читається як «все гаразд».**
Різниця лише в тому, що ціна не миттєва — дев-база не зникає, вона стає невидимою для інструменту.
...
`local-audit` рівня `warn`, текст містить `infobases:`; репо з `infobase:` → цієї знахідки немає
(інакше вона з'являлась би там, де все правильно).

**Крок дає код, а не текст.** Рев'ю Task 5 механічне (guard-`grep`-и), але цей Step дивиться як на
код: є `elseif`, є тест, є ризик хибного спрацювання на правильному репозиторії.
...
**Живий репозиторій для цього не годиться, і це не деталь.** Прогін B4 показав: `SMP_BankExchange`
ще не мігрований — маніфесту немає, натомість три `storage.json`, — тож усі команди впираються в
префлайт, і хук показав би саме його відмову, а не стан сховищ. Живі кроки нижче виконувати **на
клоні з доданим маніфестом** (як це робив компаньйон) або на синтетичному репо фікстури. Міграція
живих репозиторіїв — B6, і до неї цей шлях закритий.
